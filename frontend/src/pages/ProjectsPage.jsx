...
function MemberAvatars({ members=[] }) {
  ...
  return (
    <div style={{ display:'flex', alignItems:'center' }}>
      {show.map((m,i)=>(
        <div key={m.id} style={{ width:24, height:24, borderRadius:'50%', background:'#d1fae5', color:'#065f46', display:'flex', alignItems:'center', justifyContent:'center', fontSize:9, fontWeight:700, border:'2px solid var(--card)', marginLeft:i===0?0:-6, flexShrink:0 }}>
          {m.name?.split(' ').map(w=>w[0]).join('').toUpperCase().slice(0,2)}
        </div>
      ))}
      {extra > 0 && (
        <div style={{ width:24, height:24, borderRadius:'50%', background:'var(--muted)', color:'var(--muted-foreground)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:9, fontWeight:700, border:'2px solid var(--card)', marginLeft:-6 }}>
          +{extra}
        </div>
      )}
    </div>
...
export default function ProjectsPage() {
  ...
  const fetchProjects = (pg=page) => {
    api.get(`/projects?search=${search}&status=${status}&page=${pg}&limit=${LIMIT}`)
      .then(({ data }) => {
  ...
  useEffect(() => { setPage(1); fetchProjects(1); }, [search, status]);
        ...
        <div className="page">
          <div className="page-header">
            <div>
              ...
              <p style={{ fontSize:13, color:'var(--muted-foreground)', margin:'2px 0 0' }}>{total} projects total</p>
            </div>
          ...
          <div style={{ display:'flex', gap:10, marginBottom:20 }}>
            ...
            <select style={{ border:'1px solid var(--border)', borderRadius:8, padding:'8px 12px', fontSize:13, background:'var(--background)', color:'var(--foreground)' }} value={status} onChange={e=>setStatus(e.target.value)}>
          ...
          {projects.length === 0 ? (
            <div className="empty-state">
              <div className="empty-state-icon">
                <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M3 7a2 2 0 0 1 2-2h3l2 2h9a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/></svg>
              </div>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(3, 1fr)', gap:16 }}>
              {projects.map(p=>(
                <div key={p.id} className={`project-card ${statusClass[p.status]||''}`} onClick={()=>navigate(`/projects/${p.id}`)} style={{ overflow:'hidden', minWidth:0, display:'flex', flexDirection:'column' }}>
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:8, marginBottom:8 }}>
                    <h3 style={{ margin:0, fontSize:14, fontWeight:600, color:'var(--foreground)', letterSpacing:'-0.2px', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', minWidth:0, flex:1 }}>
                      {p.name}
                    </h3>
                    <div style={{ flexShrink:0 }}><StatusBadge value={p.status} /></div>
                  </div>
                  <p style={{ fontSize:13, color:'var(--muted-foreground)', margin:'0 0 14px', lineHeight:1.5, display:'-webkit-box', WebkitLineClamp:2, WebkitBoxOrient:'vertical', overflow:'hidden', wordBreak:'break-word', minHeight:'2.8em' }}>
                  ...
                  {(p.start_date || p.end_date) && (
                    <div style={{ fontSize:12, color:'var(--muted-foreground)', marginTop:4, display:'flex', alignItems:'center', gap:4, overflow:'hidden' }}>
                      <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ flexShrink:0 }}><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
                  ...
                  {user?.role === 'Admin' && (
                    <div style={{ marginTop:12, paddingTop:10, borderTop:'1px solid var(--border)', display:'flex', justifyContent:'flex-end' }}>
                      <button onClick={e=>deleteProject(p.id,e)} style={{ padding:'4px 12px', background:'#fee2e2', border:'1px solid #fca5a5', borderRadius:6, fontSize:11, fontWeight:500, cursor:'pointer', color:'#991b1b' }}>
                        Delete
                      </button>
                    </div>
                  )}
            ...
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginTop:20, padding:'0 4px' }}>
              ...
              <div style={{ display:'flex', gap:8 }}>
                <button
                  ...
                  disabled={page===1}
                  style={{ padding:'6px 14px', border:'1px solid var(--border)', borderRadius:7, fontSize:13, background:'var(--card)', color:page===1?'var(--muted-foreground)':'var(--foreground)', cursor:page===1?'not-allowed':'pointer', fontWeight:500 }}
                >
                  ← Prev
                </button>
                <button
                  ...
                  style={{ padding:'6px 14px', border:'1px solid var(--border)', borderRadius:7, fontSize:13, background:'var(--card)', color:page===totalPages?'var(--muted-foreground)':'var(--foreground)', cursor:page===totalPages?'not-allowed':'pointer', fontWeight:500 }}
                >
                  Next →
                </button>
              </div>
